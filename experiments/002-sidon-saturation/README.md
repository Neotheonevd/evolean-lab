...
第一轮演化搜索找到 `{22,30,31,33,43,58}`，给出 `M(6)≥61`。随后的独立种群推翻了暂定等式 `M(6)=61`，找到更强见证：

`{7,23,24,30,38,43}`。

该六元素 Sidon 集在 `[1,63]` 中按包含关系极大。`Saturation43.lean` 已机械核验，因此：

`M(6) ≥ 63` 为 `VERIFIED`，

其中 `M(k)` 表示一个 `k` 元 Sidon 集能够饱和的最长初始区间。

`M(6)=61` 已正式标记为 `REFUTED`。当前没有提出 `M(6)=63`，因为尚无上界证据。

此外，Lean 已完成一般结构定理。若 `A` 是 Sidon 集且 `x∉A`，则：

`A∪{x}` 仍是 Sidon 集，当且仅当 `x` 不存在任何 `x+a=b+c` 或 `x+x=b+c` 型阻塞等式。

因此 `A` 在目标集合 `U` 中按包含关系极大，当且仅当 `U\A` 的每个点都被这两类加法等式覆盖。这把“逐点试加”的程序性质转化成了可以用于一般构造和计数的数学接口。
...
2. 利用已经验证的阻塞覆盖等价刻画，寻找可参数化的显式构造族；
